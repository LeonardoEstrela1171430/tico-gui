...
import IDC.EvolActions.Impl.Additions.*;
...
public class Comparator 
{
    ...
    private List<EvolutionaryAction> checkNewClassesReturn(List<String> individuals_uris)
    {
        List<EvolutionaryAction> toReturn = new ArrayList<>();
        for(String uri : individuals_uris)
        {
            try
            {
    ...
    public List<EvolutionaryAction> checkForOntPropertiesReturn(List<String> individuals_uris)
    {
        List<EvolutionaryAction> toReturn = new ArrayList<>();
        for(String uri : individuals_uris)
        {
    ...
    public List<EvolutionaryAction> runComparatorOnIndividualsReturn (List<String> individuals_uris)
    {
        List <String> evolving_uris = SPARQLUtils.getIndividualsSPARQL(ModelManager.getManager().getEvolvingModel());
    ...
    public List<EvolutionaryAction> getEvolutionaryActions()
    {
        List <String> individuals_uris = SPARQLUtils.getIndividualsSPARQL(ModelManager.getManager().getInstanceModel());
        List<EvolutionaryAction> toReturn = new ArrayList<>();
        ...
        for(List<String> partition : partitions)
        {
            List<EvolutionaryAction> newClasses = checkNewClassesReturn(partition);
            List<EvolutionaryAction> ontProp = checkForOntPropertiesReturn(partition);
            copyIndividualsToEvolvedModel(partition);
            List<EvolutionaryAction> moreActions = runComparatorOnIndividualsReturn(partition);
            List<EvolutionaryAction> aux = Stream.of(newClasses, ontProp, moreActions).flatMap(Collection::stream).collect(Collectors.toList());
            toReturn.addAll(aux);
        }
        return toReturn;
    }

    public String executeActions(List<JSONObject> evolActions) throws JsonProcessingException {
        for(JSONObject action : evolActions){
            findClassAndExecuteActions(action);
        }

        OntModel thisModel = ModelManager.getManager().getEvolvingModel();
        Writer writer = new StringBuilderWriter();
        thisModel.write(writer, "TTL");
        String output = writer.toString();

        return output;
    }

    public void findClassAndExecuteActions(JSONObject action) throws JsonProcessingException {
        String aux = action.get("className").toString();

        switch (aux) {
            case "AddAllValuesFromRestriction" -> {
                AddAllValuesFromRestriction auxClass1 = new ObjectMapper().readValue(action.toString(), AddAllValuesFromRestriction.class);
                auxClass1.execute();
            }
            case "AddCardinalityRestriction" -> {
                AddCardinalityRestriction auxClass2 = new ObjectMapper().readValue(action.toString(), AddCardinalityRestriction.class);
                auxClass2.execute();
            }
            case "AddRestriction" -> {
                AddRestriction auxClass3 = new ObjectMapper().readValue(action.toString(), AddRestriction.class);
                auxClass3.execute();
            }
            case "AddSomeValuesFromRestriction" -> {
                AddSomeValuesFromRestriction auxClass4 = new ObjectMapper().readValue(action.toString(), AddSomeValuesFromRestriction.class);
                auxClass4.execute();
            }
            case "AddClass" -> {
                AddClass auxClass5 = new ObjectMapper().readValue(action.toString(), AddClass.class);
                auxClass5.execute();
            }
            case "AddDatatypeProperty" -> {
                AddDatatypeProperty auxClass6 = new ObjectMapper().readValue(action.toString(), AddDatatypeProperty.class);
                auxClass6.execute();
            }
            case "AddObjectProperty" -> {
                AddObjectProperty auxClass7 = new ObjectMapper().readValue(action.toString(), AddObjectProperty.class);
                auxClass7.execute();
            }
            case "AddProperty" -> {
                AddProperty auxClass8 = new ObjectMapper().readValue(action.toString(), AddProperty.class);
                auxClass8.execute();
            }
            case "CopyClass" -> {
                CopyClass auxClass9 = new ObjectMapper().readValue(action.toString(), CopyClass.class);
                auxClass9.execute();
            }
            case "CopyDatatypeProperty" -> {
                CopyDatatypeProperty auxClass10 = new ObjectMapper().readValue(action.toString(), CopyDatatypeProperty.class);
                auxClass10.execute();
            }
            case "CopyObjectProperty" -> {
                CopyObjectProperty auxClass11 = new ObjectMapper().readValue(action.toString(), CopyObjectProperty.class);
                auxClass11.execute();
            }
            case "CopyProperty" -> {
                CopyProperty auxClass12 = new ObjectMapper().readValue(action.toString(), CopyProperty.class);
                auxClass12.execute();
            }
        }
    }
