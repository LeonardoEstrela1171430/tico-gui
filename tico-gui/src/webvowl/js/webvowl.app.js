...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function (){
	  ...
	  app.initialize = function (ontologyContent = ""){
	    ...
	    if ( agentVersion > 0 && agentVersion <= 11 ) {
		  ...
		  if(ontologyContent){
			ontologyMenu.setup(loadOntologyFromParameter(ontologyContent));
		  }else {
	  ...
	  function loadOntologyFromParameter( ontologyContent ){
	    d3.select("#reloadCachedOntology").classed("hidden", true);
	    pauseMenu.reset();
	    graph.options().navigationMenu().hideAllMenus();
	    
	    if ( ontologyContent === undefined || ontologyContent.length === 0 ) {
	      loadingModule.notValidJsonFile();
	      return;
	    }
	    graph.editorMode(); // updates the checkbox
